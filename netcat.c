...
/* do inverse lookups in separate loop based on our collected forward addrs,
   since gethostby* tends to crap into the same buffer over and over */
    ...
    poop->addrs[0][sizeof(poop->addrs[0]) - 1] = '\0';  // Ensure null-termination
...
/* if you don't have all this FD_* macro hair in sys/types.h, you'll have to
  ...
  if (o_interval)
    Sleep (o_interval * 1000);		/* pause *before* sending stuff, too */
  errno = 0;			/* clear from sleep */
#ifdef WIN32
  WSASetLastError(0);
#endif
...
/* special case for multi-mode -- we'll want to send this one buffer to every
   open TCP port or every UDP attempt, so save its size and clean up stdin */
	...
	if (istty) {
		...
		if (kbhit()) {
/*			bigbuf_in[0] = getche(); */
			fgets(bigbuf_in, sizeof(bigbuf_in), stdin);
...
#endif
      if (o_interval)
	Sleep (o_interval * 1000);		/* if -i, delay between ports too */
      if (o_random)
	curport = nextport (randports);
      else
